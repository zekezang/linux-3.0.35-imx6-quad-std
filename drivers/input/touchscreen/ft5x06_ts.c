...
if(event->touch_event[i] == 0 || event->touch_event[i] == 2)
{
input_report_abs(data->input_dev, ABS_X, event->x[i]);
input_report_abs(data->input_dev, ABS_Y, event->y[i]);
...
//printk("chenbinE\n");
	...
	data->tch_data_len = FT_TCH_LEN(5);
...
printk("FT5X06 register succeed\n");
	return 0;
